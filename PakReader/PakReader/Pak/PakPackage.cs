...
namespace PakReader.Pak
{
    ...
    {
        public readonly ArraySegment<byte> UAsset;
        public readonly ArraySegment<byte> UExp;
        public readonly ArraySegment<byte>? UBulk;
                ...
                if (string.IsNullOrEmpty(exports.JsonData))
                {
                    ...
                    for (int i = 0; i < ret.Length; i++)
                    {
                        ret[i] = new JsonExport
                        {
                            ...
                            ExportValue = (FModel.EJsonType.Default) switch
        ...
        public FName[] ExportTypes
        {
            get
            {
                if (exports.ExportTypes == null)
                {
                    ...
                    using var bulk = UBulk != null ? new MemoryStream(UBulk.Value.Array, UBulk.Value.Offset, UBulk.Value.Count) : null;
                    asset.Position = 0;
                    exp.Position = 0;
                    if (bulk != null)
                        bulk.Position = 0;
        ...
        public PakPackage(ArraySegment<byte> asset, ArraySegment<byte> exp, ArraySegment<byte>? bulk)
        {
            UAsset = asset;
            UExp = exp;
            UBulk = bulk;
