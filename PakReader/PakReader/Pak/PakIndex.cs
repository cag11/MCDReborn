...
namespace PakReader.Pak
{
    public class PakIndex : IEnumerable<string>
    {
        ...
        public bool TryGetPackage(string path, out PakPackage package)
        {
            ...
            // A cache hit assigns `package` and must report success. Reporting false here meant
            // every package could be read exactly once and failed on every read after that,
            // which only stayed hidden because callers cache the decoded result themselves.
