...
namespace MCDSaveEdit.Logic
{
    public static class ItemExtensions
    {
        ...
        /// <summary>
        /// The one slot an item can be equipped into, or null where that is not a single
        /// answer: artifacts have three hotbar slots to choose between, and an item whose
        /// type is not in the loaded game content matches no category at all.
        /// </summary>
        public static EquipmentSlotEnum? soleEquipmentSlot(this Item item)
        {
            if (item.isMeleeWeapon()) { return EquipmentSlotEnum.MeleeGear; }
            if (item.isRangedWeapon()) { return EquipmentSlotEnum.RangedGear; }
            if (item.isArmor()) { return EquipmentSlotEnum.ArmorGear; }
            return null;
        }
